...
if(isset($_POST["add-car"])){
    ...
    $fileName3 = null;
    $fileName4 = null;
    if (isset($_FILES["image1"]["tmp_name"]) && ($_FILES["image1"]["tmp_name"] != "") || isset($_FILES["image2"]["tmp_name"]) && ($_FILES["image2"]["tmp_name"] != "")|| isset($_FILES["image3"]["tmp_name"]) && ($_FILES["image3"]["tmp_name"] != "") || isset($_FILES["image4"]["tmp_name"]) && ($_FILES["image4"]["tmp_name"] != "")){
        $checkImage1 = getimagesize($_FILES["image1"]["tmp_name"]);
        $checkImage2 = getimagesize($_FILES["image2"]["tmp_name"]);
        // $checkImage3 = getimagesize($_FILES["image3"]["tmp_name"]);
        // $checkImage4 = getimagesize($_FILES["image4"]["tmp_name"]);
        // si image alors ok
        if ($checkImage1 != false || $checkImage2 != false || $checkImage3 != false || $checkImage4 != false) {
            $fileName1 = slugify(basename($_FILES["image1"]["name"]));
            $fileName1 = uniqid()."-".$fileName1;

            $fileName2 = slugify(basename($_FILES["image2"]["name"]));
            $fileName2 = uniqid()."-".$fileName2;

            $fileName3 = slugify(basename($_FILES["image3"]["name"]));
            $fileName3 = uniqid()."-".$fileName3;

            $fileName4 = slugify(basename($_FILES["image4"]["name"]));
            $fileName4 = uniqid()."-".$fileName4;
            
            // On déplace le fichier dans upload/car
            if(move_uploaded_file($_FILES["image1"]["tmp_name"], _CAR_IMAGE_PATH_.$fileName1) || move_uploaded_file($_FILES["image2"]["tmp_name"], _CAR_IMAGE_PATH_.$fileName2) || move_uploaded_file($_FILES["image3"]["tmp_name"], _CAR_IMAGE_PATH_.$fileName3) || move_uploaded_file($_FILES["image4"]["tmp_name"], _CAR_IMAGE_PATH_.$fileName4)){
    ...
    if (isset($_FILES["image1"]["tmp_name"]) && $_FILES["image1"]["tmp_name"] != ""){
        $checkImage1 = getimagesize($_FILES["image1"]["tmp_name"]);
        $checkImage1 = getimagesize($_FILES["image1"]["tmp_name"]);
        // si image alors ok
        if ($checkImage != false) {
...
?>
    ...
    foreach ($errors as $error) { ?>
        ...
        <form method="POST" action="" enctype="multipart/form-data" class="form-add-car">
			...
			<input type="file" id="image1" name="image1" accept="image/png, image/jpg, image/jpeg" class="form-file">

			<input type="file" id="image2" name="image2" accept="image/png, image/jpg, image/jpeg" class="form-file">

			<input type="file" id="image3" name="image3" accept="image/png, image/jpg, image/jpeg" class="form-file">

			<input type="file" id="image4" name="image4" accept="image/png, image/jpg, image/jpeg" class="form-file">
